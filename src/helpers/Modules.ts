...
const helper: Record<string, any> = {
  open(id) {
    if (!this.check(id)) {
      console.error(`MÃ³dulo ${id} nÃ£o encontrado!`);
      return;
    }
  ...
  minimize(id) {
    ...
    if ($appdata.get(`modules.${id}.title`, "") == "") {
      console.error(`MÃ³dulo ${id} nÃ£o possui a prorpiedade "title"!`);
      return;
    }
    if ($appdata.get(`modules.${id}.icon`, "") == "") {
      console.error(`MÃ³dulo ${id} nÃ£o possui a prorpiedade "icon"!`);
      return;
    }
  ...
  get(list = null) {
    ...
    try {
      return {
        ...Object.fromEntries(
          list.map((module) => {
            const moduleData = $appdata.get(`modules.${module}`);
            return [
              module,
              moduleData
                ? { id: module, ...moduleData }
                : {
                  invalid: true,
  ...
  sort(modules, $t) {
    return Object.entries(modules as Record<string, any>)
      .sort(([k1, v1], [k2, v2]) => {
